...
from models.FourStageMotionRNN import MotionRNNPipeline, MotionRNNGraph, Stage0Model, Stage1Model
...
def train_graph():
    ...
    num_layers = 1
    s0_model = Stage0Model(num_layers, num_hidden[:1], args, is_first=True)
    s1_model = Stage0Model(num_layers, num_hidden[1:2], args)
    s2_model = Stage0Model(num_layers, num_hidden[2:3], args)
    s3_model = Stage1Model(num_layers, num_hidden[3:], args)
    model = MotionRNNPipeline(s0_model, s1_model, s2_model, s3_model, num_hidden[0], args, [P0, P1, P2, P3])
    numel = sum([p.numel() for p in model.parameters()])
    logger.print("model size: ", numel)

    sgd = flow.optim.SGD(model.parameters(), lr=0.001)

    graph_pipeline = MotionRNNGraph(model, sgd, args, [P0, P1, P2, P3])
