...
class Warp(nn.Module):
    ...
    def forward(self, info):
        ...
        q_lt = flow.cat([flow.clamp(q_lt[..., :N], 0, x.size(2) - 1), flow.clamp(q_lt[..., N:], 0, x.size(3) - 1)],
                         dim=-1).long()
        q_rb = flow.cat([flow.clamp(q_rb[..., :N], 0, x.size(2) - 1), flow.clamp(q_rb[..., N:], 0, x.size(3) - 1)],
                         dim=-1).long()
        q_lb = flow.cat([q_lt[..., :N], q_rb[..., N:]], dim=-1)
        q_rt = flow.cat([q_rb[..., :N], q_lt[..., N:]], dim=-1)

        # clip p
        p = flow.cat([flow.clamp(p[..., :N], 0, x.size(2) - 1), flow.clamp(p[..., N:], 0, x.size(3) - 1)], dim=-1)
        ...
        m = m.unsqueeze(dim=1)
        m = flow.cat([m for _ in range(x_warped.size(1))], dim=1)
        x_warped *= m
    ...
    def _reshape_x_warped(self, x_warped, neighbour):
...
class MotionGRU(nn.Module):
    ...
    def forward(self, x_t, pre_offset, mean):
        stacked_inputs = flow.cat([x_t, pre_offset], dim=1)
        update_gate = flow.sigmoid(self.update(stacked_inputs))
        reset_gate = flow.sigmoid(self.reset(stacked_inputs))
        offset = flow.tanh(self.output(flow.cat([x_t, pre_offset * reset_gate], dim=1)))
